...
from .meta_attention import MemEffAttention

class MVAttention(torch.nn.Module):
    def __init__(
        self, 
        dim: int,
        num_heads: int = 8,
        ...
        self.norm = torch.nn.GroupNorm(num_groups=groups, num_channels=dim, eps=eps, affine=True)
...
class ResnetBlock(torch.nn.Module):
    def __init__(
        ...
        self.resample = None
        if resample == 'up':
            self.resample = partial(torch.nn.functional.interpolate, scale_factor=2.0, mode="nearest")
        elif resample == 'down':
            self.resample = torch.nn.AvgPool2d(kernel_size=2, stride=2)
        
        self.shortcut = torch.nn.Identity()
        if self.in_channels != self.out_channels:
            self.shortcut = torch.nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=True)
...
class DownBlock(torch.nn.Module):
    def __init__(
        ...
        self.attns = torch.nn.ModuleList(attns)

        self.downsample = None
        if downsample:
            self.downsample = torch.nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=2, padding=1)
...
class UpBlock(torch.nn.Module):
    def __init__(
        ...
        self.upsample = None
        if upsample:
            self.upsample = torch.nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
...
class UNet(torch.nn.Module):
    def __init__(
        ...
        # first
        self.conv_in = torch.nn.Conv2d(in_channels, down_channels[0], kernel_size=3, stride=1, padding=1)
        ...
        # last
        self.norm_out = torch.nn.GroupNorm(num_channels=up_channels[-1], num_groups=32, eps=1e-5)
        self.conv_out = torch.nn.Conv2d(up_channels[-1], out_channels, kernel_size=3, stride=1, padding=1)
