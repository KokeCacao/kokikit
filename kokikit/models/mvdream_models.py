...
from .mvdream_attention import SpatialTransformer, SpatialTransformer3D
...
class MultiViewUNetModel(nn.Module):
    ...
    def __init__(
        ...
        if context_dim is not None:
            ...
            assert type(context_dim) is int, f"Context dim other than int is not currently supported, but is of type {type(context_dim)}"
        ...
        if num_attention_blocks is not None:
            ...
            warnings.warn(f"Constructor of UNetModel received num_attention_blocks={num_attention_blocks}. "
                          f"This option has LESS priority than attention_resolutions {attention_resolutions}, "
                          f"i.e., in cases where num_attention_blocks[i] > 0 but 2**i not in attention_resolutions, "
                          f"attention will still not be set.", RuntimeWarning)
    ...
    def forward(self, x, timesteps=None, context=None, y: Optional[Tensor] = None, camera=None, num_frames=1, **kwargs):
        ...
        """
        assert x.shape[0] % num_frames == 0, f"[UNet] input batch size must be dividable by num_frames! Got {x.shape[0]} % {num_frames} != 0"
