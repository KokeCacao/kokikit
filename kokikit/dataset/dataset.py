...
class Dataset:
    ...
    def _get_round_rays_o(
        ...
        phis = torch.linspace(0, 360 - (360 / batch_size), batch_size, device=device) / 180 * np.pi # [B,]
    ...
    @staticmethod
    def _get_lookat(batch_size: int, rays_o: Tensor, targets: Tensor, device: torch.device, opengl: bool = False) -> Tuple[Tensor, Tensor, Tensor]:
        # Get camera lookat vectors.

        if not opengl:
            # forward is camera --> target (as it should be)
            ...
            # forward is target --> camera (unorthodox radical)
            forward_vector = safe_normalize(rays_o - targets)
            up_vector = torch.tensor([0.0, 1.0, 0.0], device=device).unsqueeze(0).repeat(batch_size, 1)
    ...
    def _get_sphere_rays_o(
        ...
        phis = (torch.linspace(0, 360 * loops - (360 / batch_size), batch_size, device=device) % 360) / 180 * np.pi # [B,]
